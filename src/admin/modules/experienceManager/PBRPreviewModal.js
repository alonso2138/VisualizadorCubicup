export class PBRPreviewModal {
    ...
    async initializePreview(materialId) {
        ...
        // 1. Intentar obtener configuración temporal primero, luego del servidor
        let pbrSettings = null;
        
        // Buscar en configuración temporal (staging)
        if (window.tempPBRConfigs && window.tempPBRConfigs.has(materialId)) {
            pbrSettings = window.tempPBRConfigs.get(materialId);
            console.log('Using temporary PBR config:', pbrSettings);
        } else {
            // Fallback: intentar del servidor (para materiales ya confirmados)
            try {
                const res = await fetch(`/api/materials/${materialId}/preview-config`);
                if (res.ok) {
                    pbrSettings = await res.json();
                    console.log('Using server PBR config:', pbrSettings);
                }
            } catch (e) { 
    ...
    async setMaterial(sku) {
        ...
        // Determinar la ruta según si es temporal o definitivo
        let texturePath;
        if (window.tempPBRConfigs && window.tempPBRConfigs.has(sku)) {
            // Material temporal en uploadsTemp
            texturePath = `/uploadsTemp/${sku}/${sku}_Color.png`;
        } else {
            // Material definitivo en materials
            texturePath = `/materials/${sku}/${sku}_Color.png`;
        }
        
        this.material = await this.createTexturedMaterial(texturePath, this.materialOptions, this.mapToggles);
        if (this.mesh) {
    ...
    saveConfiguration(sku) {
        ...
        // Verificar si es un material temporal o definitivo
        if (window.tempPBRConfigs && window.tempPBRConfigs.has(sku)) {
            // Material temporal: guardar en memoria
            window.tempPBRConfigs.set(sku, pbrSettings);
            console.log(`Temporary PBR config saved for ${sku}:`, pbrSettings);
            
            if (window.adminInterface) {
                window.adminInterface.showNotification('Configuración PBR guardada temporalmente', 'success');
            }
        } else {
            // Material definitivo: guardar en servidor
    ...
    async createTexturedMaterial(texturePath, options = {}, toggles = {}) {
        return new Promise((resolve, reject) => {
            ...
            // Add error handling for base texture
            const handleTextureError = (err) => {
                console.warn(`Texture not found: ${texturePath}, using fallback material`);
                                ...
                                try {
                                    const mapTexture = await new Promise((resolve, reject) => {
                                        textureLoader.load(
                                            ...
                                            undefined,
                                            error => resolve(null) // Don't reject, just return null
                                        );
                                    });
                                    if (mapTexture) {
                                    ...
                                    console.warn(`PBR map not found: ${mapPath}`);
                                }
                            }
                        }
